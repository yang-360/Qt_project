...
Widget::Widget(QWidget *parent)
    : QWidget(parent)
{
    QVariant v(709);					//自定义类型容器
    qDebug()<<v.toInt();				//
    
	QVariant w("How are you! ");		//(c)
    qDebug()<<w.toString();				//(d)
    
	QMap<QString,QVariant>map;			//map中可以自定义键值，只对基础类型有效
    map["int"]=709;						//输入整数型
   ...
   //调用相应的转换函数并输出
	...
	if(slv.type()==QVariant::StringList)//QVariant 内部类型比对
    {
        QStringList list=slv.toStringList();//类型转换赋值
